...
int main(void)
{
  ...
  numOut, numIn, *goodOut, *badOut, goodOutTotal, status, netOut;
  int numLayer, *layerSize;
  UINT fSize, fBRead;
  ...
  while(1)
    {
      ...
      {
	  cerr<<endl<<"Error with IP Core"<<endl;
	  return XST_FAILURE;
      }
      ...
      {
	  cerr<<endl<<"Error in IP Core"<<endl;
	  return XST_FAILURE;
      }
      mcee /= numRowVal;
  ...
  for(i=numRowTrain+numRowVal; i<numRow; ++i)
    {
      ...
      try
      {
	  netOut= trainIns.feedforward(dIn[i]);
      }
      catch(int e)
      {
	  cerr<<endl<<"Error Initializing IP Core"<<endl;
	  return XST_FAILURE;
      }
