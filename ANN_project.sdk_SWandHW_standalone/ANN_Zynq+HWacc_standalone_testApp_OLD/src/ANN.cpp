...
//  _RxBuff_uOut   = (float*)  RX_BUFFER_UOUT_BASE;
  ...
  XFeedforward_Set_P_mode(&_HWInst,2);
  ...
  Xil_DCacheFlushRange((UINTPTR) _TxBuff_config, TX_BUFFER_CONFING_SIZE*4);

  /*
   * Stream the new configuration and wait for it
   */
  status = XAxiDma_SimpleTransfer(&_dma_config, (UINTPTR) _TxBuff_config,
                                  TX_BUFFER_CONFING_SIZE*4, XAXIDMA_DMA_TO_DEVICE);
  if (status != XST_SUCCESS)
    {
      throw int(XST_FAILURE);
    }
...
int ANN::feedforward(float *in)
{
  ...
  XFeedforward_Set_P_mode(&_HWInst,1);
  ...
  Xil_DCacheFlushRange((UINTPTR) _TxBuff_netIn, TX_BUFFER_CONFING_SIZE*4);
//  Xil_DCacheFlushRange((UINTPTR) _RxBuff_netOut, 4);
  ...
  status = XAxiDma_SimpleTransfer(&_dma_netIn, (UINTPTR) _TxBuff_netIn,
                                  TX_BUFFER_NETIN_SIZE*4, XAXIDMA_DMA_TO_DEVICE);
  if (status != XST_SUCCESS)
    {
      throw int(XST_FAILURE);
    }
...
void ANN::updateWandB()
{
  ...
  XFeedforward_Set_P_mode(&_HWInst,3);
  ...
  Xil_DCacheFlushRange((UINTPTR) _TxBuff_WandB,TX_BUFFER_WANDB_SIZE);

  /*
   * Stream the new weights and wait for it
   */
  status = XAxiDma_SimpleTransfer(&_dma_WandB, (UINTPTR) _TxBuff_WandB,
                                  TX_BUFFER_WANDB_SIZE, XAXIDMA_DMA_TO_DEVICE);
  if (status != XST_SUCCESS)
    {
      throw int(XST_FAILURE);
    }
