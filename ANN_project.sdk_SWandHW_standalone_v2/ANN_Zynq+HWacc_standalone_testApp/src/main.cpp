...
int main(void)
{
  ...
  netOut;
  UINT fSize, fBRead;
  float **dIn, **dTarget, mcee, minMcee, lastMcee, thMcee, *maxIn, *minIn;
  bool bad;
  char *fBuffer;
  ...
  try
  {
      ...
      for (i=0; i<numLayer; ++i)
	{
       ...
       */
      maxIn = new float[numIn];
      minIn = new float[numIn];
      for(i=0; i<numIn; ++i)
	{
	  maxIn[i] = CODEC_MIN;
	  minIn[i] = CODEC_MAX;
  ...
  try
  {
       ...
       */
      dIn = new float*[numRow];
      for(i=0; i<numRow; ++i)
	{
	  dIn[i] = new float[numIn];
	}
  ...
  try
  {
       ...
       */
      dTarget = new float*[numRow];
      for(i=0; i<numRow; ++i)
	{
	  dTarget[i] = new float[numOut];
	}
  ...
  for(i=0; i<numIn; ++i)
    {
       ...
       * Calculate Slope
       */
      float a = (CODEC_MAX-CODEC_MIN)/(maxIn[i]-minIn[i]);

      /*
       * Calculate y-intercept
       */
      float b = CODEC_MIN - a*minIn[i];
   ...
   *
   */
  cout<<"Training a new feed-forward Neural Network..."<<endl;
  XTime_GetTime(&time1);
  ...
  while(1)
    {
      ...
      {
	  cerr<<endl<<"Error With ANN Hardware accelerator (Back-propagation process)"<<endl;
	  return XST_FAILURE;
      }
      ...
      {
	  cerr<<endl<<"Error With ANN Hardware accelerator (Validation process)"<<endl;
	  return XST_FAILURE;
      }
      mcee /= numRowVal;
  ...
  if((time2-time1)<COUNTS_PER_SECOND)
    {
      cout<<"# Training time = "<<(float)(time2-time1)*1000000/COUNTS_PER_SECOND
	  <<" microsec"<<endl<<endl;
    }
  else
    {
      cout<<"# Training time = "<<(float)(time2-time1)/COUNTS_PER_SECOND
	  <<" sec"<<endl<<endl;
    }
  /*
   * TESTING PROCESS
  ...
  for(i=numRowTrain+numRowVal; i<numRow; ++i)
    {
	  ...
	  trainIns.CEE(dIn[i], dTarget[i]);
      }
      catch(int e)
      {
	  cerr<<endl<<"Error With ANN Hardware accelerator (Test process)"<<endl;
	  return XST_FAILURE;
      }
  ...
  if((time2-time1)*1000<COUNTS_PER_SECOND)
    {
      cout<<"# Test time = "<<(float)(time2-time1)*1000000/COUNTS_PER_SECOND
	  <<" microsec."<<endl<<endl;
    }
  else
    {
      cout<<"# Test time = "<<(float)(time2-time1)*1000/COUNTS_PER_SECOND
	  <<" millisec."<<endl<<endl;
    }
