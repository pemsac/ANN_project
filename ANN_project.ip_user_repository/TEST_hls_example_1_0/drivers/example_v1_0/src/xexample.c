...
void XExample_Set_mode(XExample *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XExample_WriteReg(InstancePtr->Axilites_BaseAddress, XEXAMPLE_AXILITES_ADDR_MODE_DATA, Data);
}

u32 XExample_Get_mode(XExample *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XExample_ReadReg(InstancePtr->Axilites_BaseAddress, XEXAMPLE_AXILITES_ADDR_MODE_DATA);
    return Data;
}
